...
# -----------------------------------------------------------------------------
# ===========BELOW IS NEWLY ADD FUNCTIONS FOR DATE CORRUPTION CHECKS===========
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

def check_date_order(variable, value):
  """Check if the value given is valid date order.
     valid date orders are ('dd-mm-yyyy', 'mm-dd-yyyy', 'yyyy-mm-dd')
     Use only hyphen and separating, disregard what separator actually used
  ...
  if (value not in ['dd-mm-yyyy', 'mm-dd-yyyy', 'yyyy-mm-dd']):
    raise Exception('Value of "%s" is not a correct date order. Valid date orders are (dd-mm-yyyy), (mm-dd-yyyy), (yyyy-mm-dd): %s' % \
                     (variable, str(value)))


# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
def check_date_separator(variable, value):
  """Check if the value given is valid separator.
  ...
  if (value not in ['-', '/', '.', ' ']):
    raise Exception('Value of "%s" is not a correct date separator. Valid date separators are (-), (/), (.), ( ): %s' % \
                     (variable, str(value)))


# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
def check_month_format(variable, value):
  """Check if the value given is valid month format.
     valid month formats are: ('digits', 'abbrs', 'full')
     Digits, Abbreviations and full spelling
     #This is not used currently but may be used for later updates to handle corrupting from letters formats to digits
  ...
  if (value not in ['digits', 'abbrs', 'full']):
    raise Exception('Value of "%s" is not a correct date separator. Valid date separators are (-), (/), (.), ( ): %s' % \
                     (variable, str(value)))


# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
def check_date_components_to_modify(variable, value):
  ...
  check_is_non_empty_string('variable', variable)
  check_is_list('value', value)
  for i in value:
    if (i not in ['day', 'month', 'year']):
      raise Exception('Value of "%s" is not a valid date component/s . Valid date component/s are day, month, year & they should be passed in a list: %s' % \
                       (variable, str(value)))


# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
def check_date_modification_methods(variable, value):
  """Check if the value given is/are valid modification method/s (correctors).
     valid modification method/s (correctors) are: ('add', 'decline', 'swap_digit','swap_comp', 'random', 'first','full_month','abbr_month')
     Component/s should be handled in a list 
  ...
  check_is_list('value', value)
  for i in value:
    if (i not in ['add', 'decline', 'swap_digit', 'swap_comp', 'random', 'first', 'full_month', 'abbr_month']):
      raise Exception('Value of "%s" is not a valid date modification (corruption) method . Valid date date modification (corruption) methods are: ' \
                       ' (add, decline, swap_digit, swap_comp, random , first, full_month, abbr_month) & they should be passed in a list: %s' % \
                       (variable, str(value)))
